...
//
//  import (
//  	"git.aladinbank.id/engineering/backend/health-check"
//  	"git.aladinbank.id/engineering/backend/health-check/checks/uptime"
//  	"net/http"
//  )
//
//  func main() {
...
type Health struct {
	...
	//
	// The value of the status field is case-insensitive and tightly related with the HTTP response code returned by the
	// health endpoint.
	...
	// In case of "warn" status, endpoints MUST return HTTP status in the 2xx-3xx range, and additional information
	...
	// component (e.g. a larger API or a microservice).
	// This is compatible with the behavior that current infrastructural tooling expects: load-balancers, service
	...
	// releaseId: (optional) in well-designed APIs, backwards-compatible changes in the service should not update a
	// version number.
	// APIs usually change their version number as infrequently as possible, to preserve stable interface.
	// However, implementation of an API may change much more frequently, which leads to the importance of having
	...
	// checks (optional) is an object that provides detailed health status of additional downstream systems and
	// endpoints which can affect the overall health of the main API.
	...
	// links (optional) is an objects containing link relations and URIs [RFC3986] for external links that MAY contain
	// more information about the health of the endpoint.
	// All values of this object SHALL be URIs.
	// Keys MAY also be URIs.
	// Per web-linking standards [RFC8288] a link relationship SHOULD either be a common/registered one or be indicated
	// as a URI, to avoid name clashes.
	// If a "self" link is provided, it MAY be used by clients to check health via HTTP response code, as mentioned
	// above.
...
// Since each sub-component may be backed by several nodes with varying health statuses, these keys point to arrays of
// objects.
// In case of a single-node sub-component (or if presence of nodes is not relevant), a single-element array SHOULD be
...
// It MAY have two parts: "{componentName}:{measurementName}", in which case the meaning of the parts SHOULD be as
// follows:
...
//   * A URI that indicates extra semantics and processing rules that MAY be provided by a resource at the other end of
//  the URI.
//     URIs do not have to be dereferenceable, however.
//     They are just a namespace, and the meaning of a namespace CAN be provided by any convenient means (e.g.
//    publishing an RFC, Swagger document or a nicely printed book).
...
type Checks struct {
	...
	//     * system
	// * A common and standard term from a well-known source such as schema.org, IANA or microformats.
	// * A URI that indicates extra semantics and processing rules that MAY be provided by a resource at the other end
	// of the URI.
	//   URIs do not have to be dereferenceable, however.
	//   They are just a namespace, and the meaning of a namespace CAN be provided by any convenient means (e.g.
	//  publishing an RFC, Swagger document or a nicely printed book).
	...
	// Clarifies the unit of measurement in which observedUnit is reported, e.g. for a time-based value it is important
	// to know whether the time is reported in seconds, minutes, hours or something else.
	// To make sure unit is denoted by a well-understood name or an abbreviation, it should be one of:
	// * A common and standard term from a well-known source such as schema.org, IANA, microformats, or a standards
	// document such as RFC 3339.
	// * A URI that indicates extra semantics and processing rules that MAY be provided by a resource at the other end
	// of the URI.
	//   URIs do not have to be dereferencable, however.
	//   They are just a namespace, and the meaning of a namespace CAN be provided by any convenient means (e.g.
	//  publishing an RFC, Open API Spec document or a nicely printed book).
	// See https://tools.ietf.org/id/draft-inadarei-api-health-check-04.html#section-4.4
	ObservedUnit string `json:"observedUnit,omitempty"`

	// status (optional) has the exact same meaning as the top-level "output" element, but for the
	...
	// That said, sometimes operational and resilience middleware needs to know more details about the health of the
	// API (which is why "checks" property provides details).
	// In such cases, we often need to indicate which particular endpoints are affected by a particular check's
	...
	// This assumes that the value can be cached and the reading typically doesn't happen in real time, for performance
	...
	// output (optional) has the exact same meaning as the top-level "output" element, but for the
	// sub-component/downstream dependency represented by the details object.
	// As is the case for the top-level element, this field SHOULD be omitted for "pass" state of a downstream
	// dependency.
...
type ChecksProvider interface {
	...
	// AuthorizeHealth asks whether the ChecksProvider authorizes Checks to be included in a Health response to this
	// request.
...
// @Description Returns the service health according to the upcoming IETF RFC Health Check Response Format for HTTP
// APIs https://tools.ietf.org/id/draft-inadarei-api-health-check-02.html
